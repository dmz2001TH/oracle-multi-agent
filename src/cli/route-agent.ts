...
import { fetchGitHubPrompt } from "../commands/wake-resolve.js";
import { parseWakeTarget, ensureCloned } from "../commands/wake-target.js";
import { cmdWakeAll, cmdSleep } from "../commands/fleet.js";
import { cmdDone } from "../commands/done.js";
import { cmdSleepOne } from "../commands/sleep.js";
import { cmdOracleList, cmdOracleAbout, cmdOracleScan, cmdOracleFleet } from "../commands/oracle.js";
import { cmdTake } from "../commands/take.js";
import { cmdBud } from "../commands/bud.js";
...
export async function routeAgent(cmd: string, args: string[]): Promise<boolean> {
  ...
  if (cmd === "done" || cmd === "finish") {
    if (!args[1]) { console.error("usage: maw done <window-name> [--force] [--dry-run]"); process.exit(1); }
  ...
  if (cmd === "take" || cmd === "handover") {
    if (!args[1]) { console.error("usage: maw take <session>:<window> [target-session]"); process.exit(1); }
    await cmdTake(args[1], args[2]);
    return true;
  }
  if (cmd === "bud") {
    const flags = parseFlags(args, {
      "--from": String, "--org": String, "--repo": String, "--issue": Number,
      "--note": String, "--fast": Boolean, "--root": Boolean, "--dry-run": Boolean,
    }, 1);
    ...
    await cmdBud(name, {
      from: flags["--from"], repo: flags["--repo"], org: flags["--org"],
      issue: flags["--issue"], note: flags["--note"], fast: flags["--fast"],
      root: flags["--root"], dryRun: flags["--dry-run"],
    });
    return true;
  }
  if (cmd === "oracle" || cmd === "oracles") {
    ...
    if (!subcmd || subcmd === "ls" || subcmd === "list") {
      ...
      const flags = parseFlags(args, {
        "--json": Boolean, "--force": Boolean, "--local": Boolean,
        "--remote": Boolean, "--all": Boolean, "--verbose": Boolean, "-v": "--verbose",
      }, 2);
      await cmdOracleScan({
        json: flags["--json"], force: flags["--force"], local: flags["--local"],
        remote: flags["--remote"], all: flags["--all"], verbose: flags["--verbose"],
      });
    } else if (subcmd === "fleet") {
      const flags = parseFlags(args, { "--json": Boolean, "--stale": Boolean, "--org": String }, 2);
