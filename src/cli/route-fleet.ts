import { cmdFleetLs, cmdFleetRenumber, cmdFleetValidate, cmdFleetSync, cmdFleetSyncConfigs } from "../commands/fleet.js";
import { cmdFleetInit, cmdFleetInitAgents } from "../commands/fleet-init.js";
import { cmdPulseAdd, cmdPulseLs } from "../commands/pulse.js";
import { cmdOverview } from "../commands/overview.js";
import { cmdMegaStatus, cmdMegaStop } from "../commands/mega.js";
import { cmdFederationStatus } from "../commands/federation.js";
import { cmdFederationSync } from "../commands/federation-sync.js";
import { cmdReunion } from "../commands/reunion.js";
import { cmdSoulSync } from "../commands/soul-sync.js";
import { cmdFleetHealth } from "../commands/fleet-health.js";
import { cmdFleetDoctor } from "../commands/fleet-doctor.js";
import { cmdFleetConsolidate } from "../commands/fleet-consolidate.js";
...
export async function routeFleet(cmd: string, args: string[]): Promise<boolean> {
  ...
  if (cmd === "fleet") {
    ...
    if (sub === "init") {
      ...
      const { loadSnapshot, latestSnapshot } = await import("../snapshot.js");
  ...
  if (cmd === "pulse") {
    ...
    if (subcmd === "add") {
      ...
      const { scanWorktrees, cleanupWorktree } = await import("../worktrees.js");
  ...
  if (cmd === "federation" || cmd === "fed") {
    ...
    if (!sub || sub === "status" || sub === "ls") {
      ...
      await cmdFederationSync({
        dryRun: args.includes("--dry-run"), check: args.includes("--check"),
        prune: args.includes("--prune"), force: args.includes("--force"),
  ...
  if (cmd === "soul-sync" || cmd === "soulsync" || cmd === "ss") {
    if (args.includes("--project")) {
      const { cmdSoulSyncProject } = await import("../commands/soul-sync.js");
