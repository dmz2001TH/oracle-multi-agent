...
import { loadFleetEntries } from "./fleet-load.js";
import { cmdSoulSync } from "./soul-sync.js";
import { cmdWake } from "./wake.js";
import { parseWakeTarget, ensureCloned } from "./wake-target.js";
...
`);
  ...
  if (opts.issue) {
    const { fetchIssuePrompt } = await import("./wake.js");
  ...
  if (opts.repo) {
    ...
    const { ensureCloned } = await import("./wake-target.js");
  ...
  if (opts.repo) {
    ...
    if (existsSync(localPsi)) {
      const { syncDir } = await import("./soul-sync.js");
