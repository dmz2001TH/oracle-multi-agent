...
import { loadConfig, buildCommand, getEnvVars } from "../config.js";
import { ensureSessionRunning } from "./wake.js";
import { FLEET_DIR } from "../paths.js";
import { saveTabOrder, restoreTabOrder } from "../tab-order.js";
...
export async function cmdFleetRenumber() {
  ...
  for (const e of regular) {
    ...
    if (newFile !== e.file) {
      ...
      writeFileSync(tmpPath, JSON.stringify(e.session, null, 2) + "\n");
...
export async function cmdFleetSync() {
  ...
  for (const e of entries) {
    ...
    if (added > 0) {
      ...
      writeFileSync(filePath, JSON.stringify(e.session, null, 2) + "\n");
    }
  }
...
export async function cmdFleetSyncConfigs() {
  const repoFleetDir = join(import.meta.url, "..", "..", "fleet");
